...
locals {
  ...
  region              = "[YOUR_REGION_NAME]"         # Replace with your region (e.g. eastus)
...
module "postgresql" {
  ...
  postgresql_version           = local.enable_postgresql ? local.postgresql_version : "15"
  sku_name                     = local.enable_postgresql ? local.postgresql_sku_name : "Standard_B1ms"
  administrator_login          = local.postgresql_admin_username
  storage_mb                   = local.enable_postgresql ? (try(tonumber(local.postgresql_storage_mb), 32768)) : 32768
  backup_retention_days        = local.enable_postgresql ? (try(tonumber(local.postgresql_backup_retention_days), 7)) : 7
  geo_redundant_backup_enabled = local.postgresql_geo_redundant_backup_enabled
  delegated_subnet_id          = local.enable_postgresql ? module.networking.postgresql_subnet_id : ""
  private_dns_zone_id          = local.enable_postgresql ? module.networking.postgresql_private_dns_zone_id : ""
}
